...
# llw 2021/11/25: add MIAD dataset
MIAD_CLASS_NAMES = ['catenary_dropper', 'electrical_insulator', 'metal_welding',
                    'nut_and_bolt', 'photovoltaic_module', 'wind_turbine', 'witness_mark']

class MIADDataset(Dataset):
    def __init__(self, c, is_train=True):
        assert c.class_name in MIAD_CLASS_NAMES, 'class_name: {}, should be in {}'.format(c.class_name,
                                                                                          MIAD_CLASS_NAMES)
        self.dataset_path = c.data_path
        self.class_name = c.class_name
        self.is_train = is_train
